...
//CMIN-220-W01
//4/16/2023
//my code starts line 112
//and I did modify the search function on line 43
...
function onYouTubeApiLoad() {
  ...
  gapi.client.setApiKey('AIzaSyAuKludspz1kqlM568p5B-vZ6yp0sQiK_I');
...
// My code starts here
let maxResultsInput = document.querySelector('.max')
const historyContainer = document.querySelector('.history')
const LOCAL_STORAGE_HISTORY_KEY = 'history'
let history = JSON.parse(localStorage.getItem(LOCAL_STORAGE_HISTORY_KEY)) || [];

function addHistory() {
  //test if it has alredy been searched before. if true cut it from the list 
  if(history.includes(searchTerm.value)) {
    let index = history.indexOf(searchTerm.value)
    history.splice(index, 1)
  }
  //add to front of array
  history.unshift(searchTerm.value)
  //limit search history to 10
  if(history.length == 10) {
    history.pop()
  }
  //save it to local storage and rebuild the list
  localStorage.setItem(LOCAL_STORAGE_HISTORY_KEY, JSON.stringify(history))
  historyConstructor()
}
function historyConstructor() {
  //clear container
  ...
  history.forEach(item => {
    ...
    button.addEventListener('click', () =>{
      searchTerm.value = button.innerText   
      search()
    })
  })
}

searchForm.addEventListener('submit', addHistory)
historyConstructor()